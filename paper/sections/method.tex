...
bimodal traffic---mean the datasets differ not only in teacher quality but also in spacing
structure. All loaders keep numeric features only, drop missing rows, and use a fixed per-dataset train/test split; the large time-series sets are strided down to keep the CPU TM tractable.
...
mode below corresponds to a column in the results tables, described here for reference.

\begin{itemize}
  \item \textbf{Baseline} (referred to as \texttt{uniform} in
  ...
  \item \textbf{Opposite-side type override} (\texttt{force\_ii}, \texttt{teacher\_sign}
    ...
    always fires the bounded Type~II, whereas \texttt{teacher\_sign} routes the feedback type by the teacher's error sign.
